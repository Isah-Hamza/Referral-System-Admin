...
const UserLog = () => {
    ...
    const { isLoading:loadingLogs, isRefetching:refetchingLogs, refetch:refetchLog}  = useQuery('logs', () => Log.UserLogs({query, page }), {
        onSuccess:res => {
            ...
            <div className="data  text-text_color mt-3">
                {
                    ...
                    <div key={idx} className={`${idx % 2 !== 1 && 'bg-[#f9f9f9]'} header grid grid-cols-5  gap-3 px-5 py-6 font-medium`}>
                        <div className="flex items-center text-center gap-2 line-clamp-1">
                            <p className='line-clamp-1' >{item.log_date}</p>
                            <p className='line-clamp-1' >{item.log_time}</p>
                        </div>
                        <p className='line-clamp-1' >{item.activity_type}</p>
                        <p className='' >{item.done_by}</p>
                        <p className='font-semibold text-center' >{item.action_performed}</p>
